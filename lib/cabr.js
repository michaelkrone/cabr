...
class CABR {
	 ...
	 * will be called with params.req: request and params.res: response for request validation, and
	 * additionally params.body for response validation and manipulation, after all other middleware
	 * has been called. The registerApp method must be called before any route handling middleware is
	 * registered that modifies the response body, also note that it may cause errors if the response
	 * body object is dereferenced in an attribute function!
	...
	guard(permissions, params) {
		return (req, res, next) => {
			params = _.defaults(params, {req, res});
...
function post(json, req, res) {
	...
	return new Promise((resolve, reject) => {
		...
		.then(user => {
			...
			.then(roles => {
				...
				.reduce((body, attr) =>
					Promise.resolve(this.rbac.attributes.validate(attr, user, roles, {req, res, body, permissions}))
						.then(can => can ? body : new Error('Unauthorized')), json)
			...
			log('error while getting user for request: %s', err);
			reject(err);
		});
	});
}
